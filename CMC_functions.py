...
def build_correlation_matrix(df,soln_folder_path,Reference,new_cols,components,save_flag=False,save_folder=True):
	...
	@@ -362,10 +432,8 @@ def build_correlation_matrix(df,soln_folder_path,Reference,new_cols,components,s
    ...
    if not any(Reference in element for element in new_cols):
        raise ValueError("The string",str(Reference)," is not present in the input columns") #'GrAtSiD'
    if not any('E' or 'U' or 'N' in element for element in new_cols):
        raise ValueError("At least one component has to be present in the input columns")
    if not any('station' in element for element in list(df.columns)):
	@@ -405,8 +473,8 @@ def build_correlation_matrix(df,soln_folder_path,Reference,new_cols,components,s
                corr[i,st,0]=dis
                corr[i,st,1]=len(indici1)
                for c in range(len(components)):
                    r=dfs[components[c]]-dfs[str(Reference)+'_'+components[c]]
                    rs=dfS[components[c]]-dfS[str(Reference)+'_'+components[c]]
