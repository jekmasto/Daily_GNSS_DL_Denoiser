...
def synth_series(x,seas_freqs,seas_amp,sto_mul,max_nsteps,magnitude_steps,GR,offset,sec_rate, \
    num_arctan,max_Aarctan,max_Darctan,num_gau,max_Agau,max_Dgau,decay_on,Adec,Tdec,mc_on,Outliers_flag, \
                 best_dist_percentage=None,best_dist_amplitudes=None,Nans_flag=None,max_consecutive_nans=None,total_nan_percentage=None,noise_level=None):
    ...
    if Nans_flag==True:
        ...
        Alloutputs=[y_full, y_sec, y_seas, y_seas_sto, y_steps, y_arctan, y_gau, y_dec, y_noise,boolean_outliers]

        for i in range(len(Alloutputs)):
            #print(i,Alloutputs[i][nan_positions])
            Alloutputs[i][nan_positions]=np.nan

        y_full, y_sec, y_seas, y_seas_sto, y_steps, y_arctan, y_gau, y_dec, y_noise,boolean_outliers=Alloutputs
...
def plausible_SSEs(x,max_Darctan,max_Aarctan): 
    ...
    # Calculate the adjusted value for Periodicity of SSE, between 0.8 and 3 years
...
def periodic_decomposition_trend(x,t_onset,amplitude,periodicity,verbose=False):
