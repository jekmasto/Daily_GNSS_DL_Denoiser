...
best_dist_percentage=None,best_dist_amplitudes=None,Nans_flag=None,max_consecutive_nans=None):
    ...
    for i in range(n):
        ...
        NN_full,y_full, y_sec, y_seas, y_seas_sto, y_steps, y_arctan, y_gau, y_dec, y_noise, step_locs_out, step_vals, frac_hh_mm_ss,boolean_outliers = synth_series(t,seas_freqs,seas_amp,sto_mul, \
...
def generate_syntethics_gan_noise(n,soln_folder_path,save_folder_path,sec_rateA,offsetA,decay_onA,sto_mulA,Outliers_flag, \
    ...
    ----------
        ...
        max_consecutive_nans: maximum number of admitterd consecutive Nans
    ...
    ### Remove prexisting files
    if os.path.isdir(save_folder_path):
        print(f"Save Folder '{save_folder_path}' already exists.")
        createN=input('Do you want to overwrite pre-existing folder? Y or N')
        if createN=='Y':
            for f in os.listdir(save_folder_path):
                os.remove(os.path.join(save_folder_path, f))
            print(f"Save Folder '{save_folder_path}' overwritten")
        else:
            raise ValueError("Please change the save folder path")
    else:
        os.mkdir(save_folder_path)
        print(f"Save Folder '{save_folder_path}' created successfully!")
    ...
    for i in range(n):
        ...
        NN_full,y_full, y_sec, y_seas, y_seas_sto, y_steps, y_arctan, y_gau, y_dec, y_noise, step_locs_out, step_vals, frac_hh_mm_ss,boolean_outliers = synth_series(t,seas_freqs,seas_amp,sto_mul, \
            max_nsteps,magnitude_steps,GR,offset,sec_rate,num_arctan,max_Aarctan, \
            max_Darctan,num_gau,max_Agau,max_Dgau,decay_on,Adec,Tdec,mc_on,Outliers_flag, \
        ...
        assert len(NN_full)==len(r_in)==len(boolean_outliers)
        ...
        r_in[nan_positions]=np.nan
        targety[nan_positions]=np.nan
        
        out = np.vstack([t,d_in,r_in,targety,boolean_outliers]).T
...
soln_folder_path='/home/giacomo/Documents/S_NEW/E_N_TS_residuals.npy'
#soln_folder_path='/home/giacomo/Documents/Denoiser_GPS/Wordwide_dataset/Residuals_tensors/E_N_Fake_residuals_3years.npy'

################ Save folder ################
component=0 #if 0 Est, if 1 North, if 2 Up
save_folder_path='/home/giacomo/Documents/DATA_synthetics/t_disps_residsF_E_N'
...
################ if generate_syntethics_real_noise ################
components=['E','N','U']
generate_syntethics(n,soln_folder_path,save_folder_path,sec_rateA,offsetA,decay_onA,sto_mulA,components,Outliers_flag, \
...
if Outliers_flag==True:
    ...
    ### Percentage ###
    best_dist_percentage = np.load('/home/giacomo/Documents/Daily_GNSS_DL_Denoiser-main/Synthethic_trajectories/outliers_distributions/Percentage_distributions.npy',allow_pickle=True)

    ### Amplitudes ###
    best_dist_amplitudes=np.load('/home/giacomo/Documents/Daily_GNSS_DL_Denoiser-main/Synthethic_trajectories/outliers_distributions/Best_distributions_amplitude_outliers.npy',allow_pickle=True)

    ### choose components 
    # 0 and 1 means horizontals (E,N); 2 means vertical (U)
    best_dist_percentage=best_dist_percentage[component]
    best_dist_amplitudes= best_dist_amplitudes[component]


    best_dist_percentage=( getattr(stats,best_dist_percentage[0]),best_dist_percentage[1],best_dist_percentage[2])
    best_dist_amplitudes=(getattr(stats,best_dist_amplitudes[0]),best_dist_amplitudes[1],best_dist_amplitudes[2])
    ...
    generate_syntethics_gan_noise(n,soln_folder_path,save_folder_path,sec_rateA,offsetA,decay_onA,sto_mulA,Outliers_flag, \
            best_dist_percentage,best_dist_amplitudes,Nans_flag,max_consecutive_nans)
else:
    generate_syntethics_gan_noise(n,soln_folder_path,save_folder_path,sec_rateA,offsetA,decay_onA,sto_mulA,Outliers_flag, \
            best_dist_percentage=None,best_dist_amplitudes=None,Nans_flag=Nans_flag)
